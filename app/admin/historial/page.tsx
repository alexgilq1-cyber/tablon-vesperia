...
export default function AdminHistorialPage() {
  ...
  async function cargarHistorial() {
    ...
    const { data: itemsData } = await supabase
      .from("catalogo")
      .select("id, titulo, categoria, imagen_url, coste")
      .in("id", itemIds);
    ...
    <main className="mx-auto min-h-screen max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
      ...
      <section className="border border-amber-950/45 bg-black/30 p-8 shadow-[0_25px_60px_rgba(0,0,0,0.45)]">
        ...
        <p className="mt-4 max-w-2xl text-amber-100/85">
          Aquí puedes ver quién ha canjeado cada objeto y si sigue en inventario
          o ya fue gastado.
        </p>
      </section>
      ...
      <section className="mt-8 space-y-4">
        ...
        {historial.length === 0 ? (
          <p className="text-amber-100">{mensaje || "Todavía no hay canjeos."}</p>
        ) : null}
      </section>
    </main>
  );
